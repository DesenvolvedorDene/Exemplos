...
begin
  ...
  if (FileExists('C:\Program Files (x86)\WinRAR\Rar.exe')) then
    ...
    ShowMessage('Módulo RAR não encontrado! Selecione-o manualmente!');
end;
...
begin
 DisableComponents;
  if (FileExists(RAREDT.Text)) and (FileExists(SourceEDT.Text)) then Begin
    ...
    Rewrite(FFile);
    Writeln(FFile,';Este arquivo contém instruções SFX.');
    Writeln(FFile,';Eles são usados para compactar a fonte');
    Writeln(FFile,';E bloqueie-o para que os AVs não possam acessá-lo.');
    Writeln(FFile);
